...
def get_fund_name(fund_id):
    with open('Fund_analysis/fund_3.1.csv', 'rb') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            if (row['fund_id'] == fund_id):
                return row['fund_name']

def deal_funddata(fund_id):
    with open('fund_title/fund_%s.csv'%(fund_id)) as fp_in:
        with open('fund_title/fund_%s_w.csv'%(fund_id), 'w') as fp_out:#[s for s in tr.text.split('\n') if len(s) >1 ]
            fp_out.writelines(line for i, line in enumerate(fp_in) if i !=10 )

    # with open('fund_title/fund_%s.csv'%(fund_id), 'rb') as csvfile:
    #     reader = csv.DictReader(csvfile)
    #     for row in reader:
    #         if (row['fund_value'] > 10)or(row['fund_value'] < -10):
    #             return row['fund_name']
...
if  __name__ == '__main__':  # 文件运行main入口
    ...
    my_funds = ['004450','001395','160716','163407','110022','002910','168203','164906','513050','000988','001668']
    # my_funds = ['513050']
    # fund_id_main = "004450"
    # analyze_one_fund(fund_id_main) #返回周几降的概率大，适合买入
    for fund_id in my_funds:
        download_one_fund(fund_id)
        deal_funddata(fund_id)
        weekday=analyze_one_fund(fund_id) #返回周几降的概率大，适合买入
        print fund_id+"  decrease day is week  "+str(weekday)



    #show_fund('160716')
    #run_variance('110022') # 计算盈亏率方差 用于评估股票波动



    # 增减率分开后去掉无数据行 2017-8-23 TODO

    # 基金挑选策略 排行  近一周  挑选 近一年增长率比较少， 成立以来最多的